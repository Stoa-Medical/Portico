...
* At startup it:
  ...
  2. Hydrates an in-memory `HashMap<WorkflowUuid, Workflow>` (or lazy-loads).
  3. Exposes a gRPC service (`RpcServer`) with one queue per Workflow.
...
* Uses `supabase-py` to subscribe to:
  ...
  * `agent_composition_requests` INSERTs
  * `workflows` INSERT/DELETEs
...
* Exposes `PlanWorkflow`, `CreateWorkflow`, `DeleteWorkflow`, `ProcessSignal` gRPC endpoints.
